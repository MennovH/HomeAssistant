...
result = sp.check_output(f'cp {AUTH_FILE} {TMP_AUTH_FILE}', shell=1)
...
if removed_tokens > 0:    
    ...
    with open(TMP_AUTH_FILE, "w") as f:
        ...
        result = sp.check_output(f'cp {TMP_AUTH_FILE} {AUTH_FILE}', shell=1)
    ...
    print(f"Remove {removed_tokens} token{'' if removed_tokens == 1 else 's'}")
else:
